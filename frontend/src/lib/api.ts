...
async function reqOnce<T>(
  ...
  try {
    ...
    (e as { isProxyError?: boolean; unreachable?: boolean }).isProxyError =
      true;
  ...
  if (!res.ok) {
    ...
    const proxyish =
      GATEWAY_STATUSES.has(res.status) || (!appJSON && res.status >= 500);
    if (!msg) {
      msg = proxyish
...
export interface StartRunRequest {
  ...
  agent?: string;
  llm?: string;
  /** Tri-state, and the distinction matters: OMIT to accept the server's default
   *  set, [] for deliberately none, [names] for an explicit choice. The server
   *  resolves it once, at run creation. */
  briefings?: string[];
}
...
export const api = {
  ...
  getEventsRange: (id: string, sid: string, from: number, to: number) =>
    req<EventDTO[]>(
      `/runs/${id}/sessions/${sid}/events?from_step=${from}&to_step=${to}`,
    ),
  getChat: (id: string, sid: string) =>
    req<ChatFeed>(`/runs/${id}/sessions/${sid}/chat`),
  ...
  getChatRange: (id: string, sid: string, from: number, to: number) =>
    req<ChatFeed>(
      `/runs/${id}/sessions/${sid}/chat?from_step=${from}&to_step=${to}`,
    ),
  ...
  // The briefing library offered by the new-run form. `default` is advisory —
  // the server decides what a run without an explicit selection gets.
  listBriefings: () => req<BriefingInfo[]>("/briefings"),
  ...
  cancelRun: (id: string) =>
    req<void>(`/runs/${id}/cancel`, { method: "POST" }),
  ...
  restartRun: (id: string) =>
    req<{ status: string; revived: number }>(`/runs/${id}/restart`, {
      method: "POST",
    }),
  ...
  startReport: async (
    id: string,
  ): Promise<{ report: RunReport; deferred: boolean }> => {
  ...
  searchRecall: (q: string, k?: number) =>
    req<RecallResults>(
      `/recall?q=${encodeURIComponent(q)}${k ? `&k=${k}` : ""}`,
    ),
  ...
  testLLM: (spec: string) =>
    req<TestLLMResult>("/about/test-llm", {
      method: "POST",
  ...
  checkWorkspaceAlias: (id: string) =>
    req<{ alias: string; cider_url: string }>(
      `/runs/${id}/workspace/check-alias`,
      {
        method: "POST",
      },
    ),
  ...
  listArtifacts: (id: string, path = "") =>
    req<ArtifactListing>(
      `/runs/${id}/artifacts${path ? `?path=${encodeURIComponent(path)}` : ""}`,
    ),
  // Flat, recursive listing of every file under a run's artifact dir (for the
  // browser's filename search).
  listArtifactsAll: (id: string) =>
    req<ArtifactAllListing>(`/runs/${id}/artifacts/all`),
};
