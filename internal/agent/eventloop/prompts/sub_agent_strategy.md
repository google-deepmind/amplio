...
- **Delegate less the deeper you are.** Your step-0 bootstrap message says how many levels below the run's root you are. Avoid delegating indefinitely. **At 3 or more levels deep, do the work yourself**. Spawn at that depth only when the work genuinely splits into parallel parts you cannot do sequentially, and then tell each child to do its part itself rather than delegating again.
